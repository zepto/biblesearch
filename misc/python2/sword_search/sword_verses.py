...
def mod_to_dbm(module, key_iter, path):
    ...
    with IndexDbm(dbm_name, 'n') as dbm_file:
        for key in key_iter:
...
def make_daily_dbm(path=INDEX_PATH):
...
def make_strongs_dbm(path=INDEX_PATH):
...
def make_robinson_dbm(path=INDEX_PATH):
...
def make_raw_kjv_dbm(path=INDEX_PATH):
...
class IndexBible(object):
    ...
    def write_index(self):
        ...
        #return
            ...
            with IndexDbm(dbm_name, 'n') as index_file:
